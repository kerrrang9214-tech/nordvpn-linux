...
func NewNetlinkMonitor(ignoreIntfs []string) (*NetlinkMonitor, error) {
	...
	nlmon.cached = mapset.NewSetFromMapKeys(device.InterfacesWithDefaultRoute(nlmon.ignored))
...
func (m *NetlinkMonitor) checkForChanges(re Reconnector) {
	interfaces := mapset.NewSetFromMapKeys(device.InterfacesWithDefaultRoute(m.ignored))
